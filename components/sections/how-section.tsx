      ...
      <AppleCard variant="glass" padding="lg" hover="lift" className="h-full">
        <div className="w-24 h-24 bg-apple-blue-light/10 rounded-full flex items-center justify-center mx-auto mb-8">
          {React.cloneElement(icon as React.ReactElement, {
            className: 'h-12 w-12 text-apple-blue-primary',
          })}
        </div>
        <h3 className={applyTypography.heading.h3('mb-4 text-apple-gray-500')}>
          {title}
        </h3>
  ...
  const approaches = [
    {
      title: 'Artistry',
      description:
        'We approach AI with the sensibility of artists, not just engineers. Every interaction is considered. Every interface is intuitive. Every experience is meaningful.',
      icon: <Palette className="h-12 w-12" />,
    },
    {
      title: 'Joy',
      description:
        'Technology should bring joy, not just utility. We design experiences that delight and inspire, not just solve problems.',
      icon: <Smile className="h-12 w-12" />,
    },
    {
      title: 'Invisible',
      description:
        'The most powerful technology disappears. Our solutions work like thought itself—present when needed, invisible when not, always in service of human intent.',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p
            className={cn(
              typography.body.large,
              'italic text-apple-gray-300 font-light'
            )}
          >
